...
const createEnquiry = catchAsync(async (req, res) => {
  ...
  if (body.client_id || (body.client && body.client.id)) {
    ...
    const contact_number =
      body.client?.contact_number ?? body.contact_number ?? null;
  ...
  // If frontend provided a new venue name in `new_venue_name` (string), create it via venue service
  const venueSvc = services.get('venue');
  let venueIdToUse = body.venue_id ? Number(body.venue_id) : null;
  if (body.new_venue_name && typeof body.new_venue_name === 'string' && body.new_venue_name.trim()) {
    try {
      const createdVenue = await venueSvc.create({ venue: body.new_venue_name, created_by: req.user?.id || null });
      venueIdToUse = createdVenue?.id ? Number(createdVenue.id) : venueIdToUse;
    } catch (e) {
      console.error('[createEnquiry] create venue failed', e?.message || e);
    }
  }
  ...
  const eventData = {
    ...
    ...(Number.isFinite(venueIdToUse) && venueIdToUse > 0
      ? { venues: { connect: { id: venueIdToUse } } }
      : {}),
    ...(Number.isFinite(djIdNum) && djIdNum > 0
      ? { users_events_dj_idTousers: { connect: { id: djIdNum } } }
      : {}),
    ...
    dj_cost_price_for_event:
      body.dj_cost_price != null ? Number(body.dj_cost_price) : null,
  ...
  const created = await prisma.$transaction(async (tx) => {
    ...
    // Normalize bracketed form-data structures and accept snake_case or camelCase keys
    const normalizeArrayField = (obj, keys) => {
      for (const k of keys) {
        const field = obj[k];
        if (Array.isArray(field)) return field;
        if (field && typeof field === 'object') return Object.keys(field).map(i => field[i]);
      }
      return [];
    };
    ...
    const rigNotesArray = normalizeArrayField(body, ['rigNotesData','rig_notes_data','rigNotes_data','rig_notesData']);

    // helper to create packages array items
    const makePackage = (p) => ({
      equipment_id: p.equipment_id ? Number(p.equipment_id) : null,
      equipment_order_id:
        p.equipment_order_id !== undefined
          ? Number(p.equipment_order_id)
          : null,
...
// List enquiries (supports simple pagination via query: page, perPage)
const listEnquiries = catchAsync(async (req, res) => {
  ...
  res.json(
    serializeForJson({ success: true, data: events, meta: { page, perPage } }),
  );
});

// Open enquiry: list open enquiries (event_status_id = 1)
const getOpenEnquiry = catchAsync(async (req, res) => {
...
const sendUsrBrochure = catchAsync(async (req, res) => {
  const body = req.validated || req.body || {};
  ...
  res.json(
    serializeForJson({ message: "Brochure Email sent", event: created }),
  );
});

const sendUsrUpdateEmail = catchAsync(async (req, res) => {
  const body = req.validated || req.body || {};
  const userId = Number(body.id || body.userId);
  const eventId = Number(body.eventId || body.event_id || body.id);
  const companyName = body.companyName || null;
...
const addDepositStore = catchAsync(async (req, res) => {
  const body = req.validated || req.body || {};
  const required = [
    "event_id",
    "payment_method_id",
