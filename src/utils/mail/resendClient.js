...
async function sendEmail({ to, cc, subject, html, attachments }) {
  ...
  if (!client) {
    console.log('[resend-fallback] to=', to, 'cc=', cc, 'subject=', subject, 'attachments=', Array.isArray(attachments) ? attachments.length : 0);
  ...
  if (cc && (Array.isArray(cc) ? cc.length : true)) payload.cc = cc;
