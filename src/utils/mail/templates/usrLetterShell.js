...
// send_quote/open_enquiry_invoice/thankyou_mail all render this exact same
// "Hi {name} / body / logo / company address / contact block" shell with
// only the body text (and sometimes a link) differing. Node had several
// one-off, inconsistent versions of this instead of one shared shell — none
// matched Laravel's actual look.
export function buildUsrLetterEmail({ name, bodyHtml, company, logoUrl }) {
  const addressLines = [];
