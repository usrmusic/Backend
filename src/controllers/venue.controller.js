...
const listVenues = catchAsync(async (req, res) => {
  ...
  res.json({ data: serializeForJson(venues), meta: { total: count, perPage, page, totalPages } });
});
...
const createVenue = catchAsync(async (req, res) => {
  const {
    venue, stage, power, access, smoke_haze, rigging_point, venue_address, notes, created_by
  } = req.body || {};
...
const deleteVenue = catchAsync(async (req, res) => {
  ...
  if (hasEvent)
    return res.status(400).json({ error: 'venue_has_events', message: 'Cannot delete venue. Venue has associated events.' });
...
const deleteManyVenues = catchAsync(async (req, res) => {
  ...
  let ids = [];
  if (req.params && req.params.ids) ids = String(req.params.ids).split(',').map((s) => s.trim());
  else if (Array.isArray(req.body && req.body.ids)) ids = req.body.ids;
  else if (req.body && typeof req.body.ids === 'string') ids = req.body.ids.split(',').map((s) => s.trim());
  ...
  if (numericIds.length === 0) return res.status(400).json({ error: 'invalid_ids' });
  ...
  for (const _id of numericIds) {
    ...
    if (hasEvent)
      return res.status(400).json({ error: 'venue_has_events', message: `Cannot delete venue ${_id}. Venue has associated events.` });
  }
...
export default { listVenues, getVenue, createVenue, updateVenue, deleteVenue, deleteManyVenues, listVenueDropdown };
